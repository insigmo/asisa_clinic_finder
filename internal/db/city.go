...
const queryFindCityPostalCodes = `
	SELECT postal_code
...
func (db *Manager) FindCity(ctx context.Context, city string) ([]int, error) {
	rows, err := db.client.QueryContext(ctx, queryFindCityPostalCodes, city)
	if err != nil {
		return nil, fmt.Errorf("select city_postal_codes failed: %w", err)
	}
	defer rows.Close()

	postalCodes := make([]int, 0, 5)
	for rows.Next() {
		var postalCode int
